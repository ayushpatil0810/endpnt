 ...
 */
export const getGithubProfile = cache(async (username: string): Promise<GithubProfileData | null> => {
  if (!username) return null;

  const token = process.env.GITHUB_ACCESS_TOKEN;
  
  if (!token) {
    console.warn("GITHUB_ACCESS_TOKEN is not set. GitHub Identity Layer will gracefully degrade.");

...
export async function fetchMediumPosts(username: string): Promise<BlogPost[]> {
  try {
    ...
    for (const match of itemMatches) {
      ...
      const description = stripHtml(extractXml(item, "description") ?? "").slice(0, MAX_DESCRIPTION_LENGTH);
...
export async function fetchHashnodePosts(username: string): Promise<BlogPost[]> {
  try {
    ...
    const edges: Array<{ node: { title: string; url: string; publishedAt: string; brief?: string } }> =

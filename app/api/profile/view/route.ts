...
export async function POST(request: NextRequest) {
	try {
		...
		// Cookie-based deduplication — prevents counting the same visitor twice
		// within a 24-hour window without requiring user authentication.
		...
		// Redis INCR + async Postgres event insert (non-blocking via after())
		trackProfileView({
			userId: user.id,
			username,

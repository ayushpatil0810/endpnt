...
export async function generateMetadata({ params }: ProfilePageProps): Promise<Metadata> {
	...
	const description =
		user.seoDescription || user.bio || `Check out ${user.username}'s profile on Endpoint`;
...
export default async function ProfilePage({ params }: ProfilePageProps) {
	...
	const [userLinks, userProjects] = await Promise.all([
		db.select().from(links).where(eq(links.userId, user.id)).orderBy(asc(links.displayOrder)),
		db
			.select()
			.from(projects)

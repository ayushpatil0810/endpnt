...
export async function GithubTopRepos({ username, theme }: { username: string; theme: ThemeDefinition }) {
    ...
    <ThemedCard theme={theme} className="w-full p-5 sm:p-6 flex flex-col h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 flex-1">
        {profile.pinnedRepos.map((repo) => (
          ...
          >
            <span className="font-semibold text-sm tracking-tight truncate group-hover:underline" style={{ color: "var(--theme-text-primary)" }}>
              {repo.name}
            </span>
            <p className="text-xs line-clamp-2 min-h-[32px]" style={{ color: "var(--theme-text-secondary)" }}>
              {repo.description || "No description provided."}
            </p>
            <div className="flex items-center gap-3 text-[10px] mt-auto pt-2 font-mono" style={{ color: "var(--theme-text-secondary)" }}>

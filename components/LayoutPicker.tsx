...
import { IconCheck, IconLoader2, IconLayoutSidebar, IconLayoutDashboard, IconLayoutAlignCenter } from "@tabler/icons-react";
        ...
        {LAYOUTS.map((layout) => {
          ...
          return (
            ...
            >
              ...
              <div
                className="w-full h-28 rounded-lg overflow-hidden flex items-center justify-center relative shadow-sm border border-border/50 bg-background/50"
              >
                {isActive && (
                ...
                <Icon size={40} className="text-muted-foreground group-hover:text-foreground transition-colors opacity-50" stroke={1.5} />
              </div>

              {/* Info */}
              <div className="flex flex-col gap-1.5 z-10 w-full">
                <span className="text-sm font-semibold text-foreground">{layout.name}</span>
                <p className="text-[11px] text-muted-foreground leading-snug">{layout.description}</p>
              </div>
            </button>
          );
        })}

...
export async function GithubLanguages({ username, theme }: { username: string; theme: ThemeDefinition }) {
  ...
  return (
    <ThemedCard theme={theme} className="w-full p-5 sm:p-6 h-full flex flex-col gap-4">
      ...
      <div className="flex w-full h-2 rounded-full overflow-hidden" style={{ backgroundColor: "var(--theme-separator)" }}>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-3">
        {profile.languages.map((lang) => (
          <div key={lang.name} className="flex items-center gap-1.5 text-xs">
            ...
            <span className="font-medium" style={{ color: "var(--theme-text-primary)" }}>{lang.name}</span>
            <span className="font-mono text-[10px]" style={{ color: "var(--theme-text-secondary)" }}>{Math.round(lang.percentage)}%</span>
          </div>
        ))}
      </div>
    </ThemedCard>

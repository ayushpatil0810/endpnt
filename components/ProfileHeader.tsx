...
export function ProfileHeader({ username, bio, avatarUrl, onBioUpdate }: ProfileHeaderProps) {
			...
			<div className="flex-1 w-full flex flex-col items-start min-w-0">
				...
				{editing ? (
					...
					<div className="mt-4 text-sm text-foreground/80 font-mono tracking-wide leading-relaxed max-w-xl normal-case">
						{bio || <span className="opacity-50 italic">No bio yet — click edit to add one.</span>}
					</div>
				)}
			</div>
		</div>
